...
public class Game implements Identifiable {
	...
	public Game(
		...
		} else if (player1.getPlayerId() == currentPlayerId) {
			this.currentPlayer = player1;
		} else if (player2.getPlayerId() == currentPlayerId) {
			this.currentPlayer = player2;
		} else {
			throw new IllegalArgumentException(
					format("expected current player id to be one of <%d> or <%d>, but got <%d>", player1.getPlayerId(), player2.getPlayerId(), currentPlayerId));
		}
		this.board = board;
	...
	public boolean isPlayer(Profile playerProfile) {
		...
		if (player2.getPlayerId() == playerProfile.getId()) {
			return true;
		}
		return false;
	}
